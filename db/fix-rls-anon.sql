...
DROP POLICY IF EXISTS "hof_write"     ON hof_results;
...
CREATE POLICY "events_write" ON events FOR ALL    USING (TRUE) WITH CHECK (TRUE);
CREATE POLICY "events_read"  ON events FOR SELECT USING (TRUE);

-- ── session_templates ─────────────────────────────────────────────────────────
DROP POLICY IF EXISTS "templates_admin_all" ON session_templates;
DROP POLICY IF EXISTS "templates_write"     ON session_templates;
DROP POLICY IF EXISTS "templates_read"      ON session_templates;
DROP POLICY IF EXISTS "templates_select"    ON session_templates;
CREATE POLICY "templates_write" ON session_templates FOR ALL    USING (TRUE) WITH CHECK (TRUE);
...
CREATE POLICY "signups_write" ON signups FOR ALL    USING (TRUE) WITH CHECK (TRUE);
...
DROP POLICY IF EXISTS "handicaps_read"      ON handicap_history;
DROP POLICY IF EXISTS "handicaps_write"     ON handicap_history;
