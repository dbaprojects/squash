...
| v6.08 | Admin Ladder challenges: ↩ Undo button on completed rows (super_admin); `undoChallengeResult()` reverses position cascade using stored `winner_pos_change`, restores challenge to `accepted`, restores challenges voided within 60s window back to `pending`/`accepted` |
