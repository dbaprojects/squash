...
| v6.03 | Voided challenge shows who caused it: `_voidOutOfRangeChallenges(causedBy)` writes the cause to `message` field; history row shows "🐌 X vs Y voided — David B. won their match"; `_pendingVoidCause` module var passes cause through `loadDivisionLadder` → `_processAutoForfeits`; completed query now fetches `message` |
